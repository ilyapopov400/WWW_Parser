...
class ParserCard:
    ...
    def _row(self):
        ...
        return [name, description[0], description[2], description[4], description[3], price, ]
...
def get_result_list(url, parser) -> list:  # получаем список по url с использованием класса парсера
    ...
    while True:
        count_page = 1
        while True:
            page_url = url.format(type_page, count_page)
            ...
            count_page += 1
        if not GetHtml(url=url.format(type_page, count_page)).get_html():
            break
