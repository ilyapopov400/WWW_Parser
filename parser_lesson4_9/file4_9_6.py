...
class ParserCard:
    ...
    def _row(self):
        ...
        article = f(card.select_one('p.article'))

        description = card.select_one('#description').select('li')

        description = list(map(f, description))

        in_stock = f(card.select_one('#in_stock'))
        price = card.select_one('#price').text.strip()
        old_price = card.select_one('#old_price').text.strip()
        return [name, article, *description, in_stock, price, old_price, ]
