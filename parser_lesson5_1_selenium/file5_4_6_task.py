...
with webdriver.Chrome(options=options_chrome) as driver:
    ...
    forms = filter(lambda x: int(x.get_attribute("value")) in numbers, forms)

    [x.click() for x in forms]

    form = driver.find_element(By.CSS_SELECTOR, "body")
    form = form.find_element(By.CLASS_NAME, "main")
    form = form.find_element(By.CLASS_NAME, "btn_box")
    form = form.find_element(By.CLASS_NAME, "btn")
    form.click()
