...
class Parser:
    def __init__(self, url: str):
        ...
        self.result = dict()
        self.cookies = [{'name': 'KXIYO4xMrWh', 'value': 'ibyAZPfXAsPqptPaNyL'},
    ...
    def _data_of_person(self, driver, cookie):
        count_skills = len(driver.find_element(By.ID, "skills").text)
        name = driver.find_element(By.ID, "name").text.split(': ')[-1]
        age = int(driver.find_element(By.ID, "age").text.split(': ')[-1])

        self.result[name] = [age, count_skills, cookie.get("name"), cookie.get("value")]

    def _parser(self):
        with webdriver.Chrome(options=self.options_chrome) as driver:
            driver.get(URL)
            for cookie in tqdm(self.cookies):
                driver.delete_all_cookies()
                # time.sleep(1)
                driver.add_cookie(cookie)
                driver.refresh()
                self._data_of_person(driver=driver, cookie=cookie)

            with open(file='data_parser.txt', mode="w", encoding="utf-8") as file:
                json.dump(self.result, file)
            time.sleep(3)
            return self.result

    def _filter_date(self, dict_result):
        age_min = min(dict_result.values(), key=lambda x: x[0])[0]
        res = dict()
        ...
        scill_maxx = max(res.values(), key=lambda x: x[1])[1]
        result = dict()
        for key, value in res.items():
            if value[1] == scill_maxx:
                result[key] = value
    ...
    def run(self):
        ...
        if os.path.exists("data_parser.txt"):
            with open(file='data_parser.txt', mode="r", encoding="utf-8") as file:
                dict_result = json.load(file)
        else:
            dict_result = self._parser()

        result = self._filter_date(dict_result)
        return result
